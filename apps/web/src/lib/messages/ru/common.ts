const common = {
  navigation: {
    itemUnavailable: 'Сохранённый объект больше недоступен. Фильтры сохранены.',
    loadFailed: 'Не удалось загрузить ваши данные',
    refreshFailed: 'Не удалось обновить весь список. Предыдущий вид сохранён.',
    originUnavailable: 'Предыдущий объект недоступен. Открыт родительский раздел.',
    returnFailed: 'Не удалось вернуться. Попробуйте ещё раз.',
    source: 'Источник',
    returnTo: 'Назад: {place}',
    reset: 'Сбросить вид',
    storageUnavailable: 'Место работы сохранится при переходах, но восстановление после перезагрузки сейчас недоступно.',
    positionUnavailable: 'Точное место восстановить не удалось. Фильтры сохранены.',
    retry: 'Повторить восстановление',
  },
  app: {
    cardCount: 'Карточек: {n}',
    name: 'Reomi',

    tagline: 'Anki на стероидах — граф, сад, AI.',
    workspace: 'мозг {name}',
  },
  nav: {
    home: 'Главная',
    review: 'Повтор',
    graph: 'Граф',
    decks: 'Колоды',
    cards: 'Карточки',
    chat: 'Чат',
    library: 'Библиотека',
    notebooks: 'Блокноты',
    garden: 'Сад',
    editor: 'Редактор',
    stats: 'Статистика',
    settings: 'Настройки',
    sections: {
      memory: 'Память',
      knowledge: 'Знания',
    },
  },
  topbar: {
    switchToLight: 'Включить светлую тему',
    switchToDark: 'Включить тёмную тему',
    newCard: 'Новая карточка',
    searchPlaceholder: 'Поиск карт, тегов, тем…',
    menuLabel: 'Меню',
    searchLabel: 'Поиск',
    newCardLabel: 'Новая карточка',
  },
  actions: {
    save: 'Сохранить',
    cancel: 'Отмена',
    delete: 'Удалить',
    create: 'Создать',
    rename: 'Переименовать',
    recolor: 'Сменить цвет',
    back: 'Назад',
    next: 'Далее',
    finish: 'Готово',
    continue: 'Продолжить',
    skip: 'Пропустить',
    open: 'Открыть',
    close: 'Закрыть',
    confirm: 'Подтвердить',
    edit: 'Редактировать',
    add: 'Добавить',
    ok: 'ОК',
  },
  units: {
    card: 'карта',
    cards: 'карт',
    deck: 'колода',
    decks: 'колод',
    day: 'день',
    days: 'дней',
    min: 'мин',
    minutes: 'минут',
    hour: 'час',
    hours: 'часов',
    second: 'секунда',
    seconds: 'секунд',
    xp: 'XP',
    level: 'Уровень',
    botanist: 'Ботаник',
  },
  time: {
    now: 'сейчас',
    today: 'Сегодня',
    tomorrow: 'Завтра',
    yesterday: 'Вчера',
    ago: '{n}{unit} назад',
    noSessions: 'сессий ещё не было',
    justNow: 'только что',
  },
  auth: {
    signOut: 'Выйти',
    signingOut: 'Выходим…',
  },
  chrome: {
    resizeSidebar: 'Изменить ширину боковой панели',
    toggleSidebar: 'Скрыть/показать панель',
    wcoHintTitle: 'Слить шапку с окном',
    wcoHint: 'Нажмите ⌄ в заголовке окна — тулбар приложения займёт его место. Достаточно одного раза, выбор запомнится.',
  },
  states: {
    loading: 'Загрузка…',
    noData: 'Нет данных',
    todo: 'TODO',
  },
  selectSearch: 'Поиск…',
  noResults: 'Ничего не найдено',
  toasts: {
    freeze: {
      title: 'Стрик сохранён',
      description: 'Использована заморозка — серия продолжается.',
    },
    dailyGoal: {
      title: 'Дневная цель выполнена',
      description: 'Так держать — стрик и XP идут дальше.',
    },
    leech: {
      title: 'Карточка приостановлена',
      description: 'Слишком много ошибок — карточка стала «пиявкой» и приостановлена.',
    },
    error: 'Не удалось сохранить — попробуйте ещё раз.',
  },
};

export default common;
