...
export function countTagEdges(cards: Card[]): number {
  let count = 0;
  for (let i = 0; i < cards.length; i++) {
    for (let j = i + 1; j < cards.length; j++) {
      if (cards[i].tags.some((tag) => cards[j].tags.includes(tag))) count++;
    }
  }
  return count;
}

export function getGraphSummary(cards: Card[], decks: Deck[]) {
  return {
    nodes: cards.length,
    links: countTagEdges(cards),
    clusters: decks.length,
  };
}
