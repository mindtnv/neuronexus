...
export const NNLeagues = () => {
  ...
  const myName = profile?.name ?? 'Alex';
  const myXp = profile?.xp ?? 2847;
  const myStreak = profile?.streakDays ?? 24;
  const myInitials = myName.trim().charAt(0).toUpperCase() || 'A';
  ...
  const rows = useMemo(() => [
    ...
    { rank: 4, n: myName,   xp: myXp,   streak: myStreak, me: true, tone: 'lime', avatar: '#9ad155', initials: myInitials },
  ...
  ], [myName, myXp, myStreak, myInitials]);
  return (
    <div style={{
      flex: 1,
      display: 'grid',
