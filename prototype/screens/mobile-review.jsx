...
// ─────────────────────────────────────────────
const HapticRipple = ({ trigger, color }) => {
...
// Swipe card shell
// ─────────────────────────────────────────────
const SwipeCard = ({ children, onSwipe, revealed }) => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        {transcript
          ...
          : <span style={{ fontSize: 13, color: 'var(--text-dim)' }}>Listening…</span>
        }
      </div>
      ...
      <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Tap to stop</div>
    </div>
  );
};
...
const MobReviewFullscreen = () => {
  ...
  const GRADES = [
    { id: 'again', label: 'Again', interval: '<1m', color: 'var(--rose-400)',  bg: 'rgba(209,85,102,0.1)',  border: 'rgba(209,85,102,0.35)' },
    { id: 'hard',  label: 'Hard',  interval: '8m',  color: 'var(--amber-400)', bg: 'rgba(243,182,85,0.1)',  border: 'rgba(243,182,85,0.35)' },
    { id: 'good',  label: 'Good',  interval: '3d',  color: 'var(--lime-400)',  bg: 'rgba(154,209,85,0.1)',  border: 'rgba(154,209,85,0.35)' },
    { id: 'easy',  label: 'Easy',  interval: '9d',  color: 'var(--sky-400)',   bg: 'rgba(85,196,214,0.1)',  border: 'rgba(85,196,214,0.35)' },
  ];
    ...
    <div style={{
      ...
      {!revealed && (
        <div style={{
          ...
          <span>← Again</span>
          <span style={{ opacity: 0.4 }}>swipe to grade</span>
          <span>Good →</span>
        </div>
      )}
        ...
        <SwipeCard onSwipe={handleSwipe} revealed={revealed}>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 18 }}>
              {card.tags.map(([c, l]) => <NNTag key={l} color={c}>{l}</NNTag>)}
            </div>
      ...
      <div style={{
        ...
        {[
          ...
          <div key={t.id} style={{
            ...
            color: t.id === 'review' ? 'var(--lime-400)' : 'var(--text-dim)',
...
// ─────────────────────────────────────────────
const NNMobileReviewDetail = () => {
  const states = [
    { label: 'Question · mic to answer',   revealedInit: false, micInit: false  },
    { label: 'Revealed · swipe or grade',  revealedInit: true,  micInit: false  },
    { label: 'Voice answer · mic overlay', revealedInit: false, micInit: true   },
  ];
...
const MobReviewStaticSnapshot = ({ revealedInit, micInit }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {[{ id: 'home', i: 'home', l: 'Home' }, { id: 'review', i: 'bolt', l: 'Review' }, { id: 'graph', i: 'graph', l: 'Graph' }, { id: 'garden', i: 'garden', l: 'Garden' }].map(t => (
          <div key={t.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, color: t.id === 'review' ? 'var(--lime-400)' : 'var(--text-dim)', padding: '6px 10px' }}>
            <NNIcon name={t.i} size={22}/>
            <span style={{ fontSize: 10.5, fontWeight: 500 }}>{t.l}</span>
          </div>
        ))}
...
const NNMobileInteractive = () => (
  <IOSDevice width={390} height={844} dark>
    <MobReviewFullscreen/>
  </IOSDevice>
);

Object.assign(window, { NNMobileReviewDetail, NNMobileInteractive, MobReviewFullscreen });
