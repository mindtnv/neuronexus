...
## Local editor drafts (R2-025)

Use versioned localStorage records scoped by owner + note/type ID, with separate `new` slots. UUIDv7 revision tokens prevent observed stale writers/late receipts from replacing or clearing newer work. Limits are 256 KiB serialized UTF-8 per draft, ten entries and 1 MiB per owner. No eviction or truncation: failures stay visible, retain the live input, and offer direct JSON download; capacity management can open in a second tab without losing the current buffer. This is local recovery, not offline server editing or a distributed merge protocol.

Autosave runs after 300 ms and flushes on pagehide, beforeunload and editor unmount. Native history/reload retains the local text; app links, programmatic navigation and explicit browser-dock switches use the existing navigation provider's editor guard. Save/keep/discard/stay reuse the dialog UI. A confirmed API receipt clears only the matching slot, including after the original editor unmounts, while newer drafts and other accounts stay intact. Ownership is fixed per mounted editor; bootstrap must finish before editable fields mount. Pending restore choices disable edits and saves. Note snapshots retain their source version and definition; type drafts retain their version and kind. Restoring an unchanged deck choice adopts the currently opened card's deck instead of implicitly moving siblings back.

A small local draft list exposes owned text/downloads even when the original entity was deleted, and permits explicit revision-checked cleanup. Unknown envelope formats are not rendered. Actual definition conflicts use the existing review-current-version workflow; retained unmapped fields are shown instead of hidden, with explicit removal when necessary. There is no server table, worker or session-history replacement.


## Required session-recovery slice for navigation continuity

`preserve-navigation-context` depends on R2-049/050/051/052/059. Implement this slice in the existing review owner, rather than a second navigation ledger. Versioned `POST /reviews/operations/:operationId` grades and `GET` receipts prevent an older API from silently accepting an idempotency field it cannot honor; legacy `POST /reviews` stays compatible. A new additive `review_operations` table stores owner + operation UUID, canonical argument hash and the committed response in the grading transaction. A unique owner/operation index and the existing profile lock serialize retries. The nullable review FK becomes a permanent reverted receipt when undo or card deletion removes its review; replay must never recreate that grade. No queue/Redis is introduced.

The browser's existing study-session module owns a bounded, account-scoped checkpoint with current answer state, confirmed review IDs and a pending operation reference. On reload, reauthentication or focus/wake, it reconciles the current card and any pending receipt before allowing grading. Recovery never sends a grade automatically. Explicit retry reuses its original operation ID and arguments. Time away/sleep is excluded from active-answer duration; current server time is refreshed on wake. Navigation stores only the opaque session ID.

Deploy the compatible API/migration before this slice's web client. Roll back web first; leave the additive operation table/receipts in place. The old grade route remains usable by old clients. User deletion cascades operation receipts. New database IDs use UUIDv7; no provider keys or additional environment configuration are needed. This dependency slice does not complete the rest of the 120-item SRS audit.

Verification for the R2-049/050/051/052/059 dependency slice is recorded in `../archive/2026-09-23-preserve-navigation-context/acceptance.md`: migration-faithful full suite, receipt concurrency/undo isolation, all-engine recovery journeys, same-owner 401 recovery, peer edits and controlled wake/clock tests. Other items in this change remain independently tracked and open.
