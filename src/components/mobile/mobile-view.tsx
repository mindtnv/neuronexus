...
function usePulledData() {
  ...
  const dueCount = useMemo(
    () => cards.filter((c) => new Date(c.fsrs.due).getTime() <= now).length,
    [cards, now],
  );
  const dueByDeck = useMemo(() => {
    const map = new Map<string, number>();
    for (const c of cards) {
      if (new Date(c.fsrs.due).getTime() <= now) {
  ...
  const firstDue = useMemo(
    () =>
      cards
        ...
        .sort((a, b) => new Date(a.fsrs.due).getTime() - new Date(b.fsrs.due).getTime())[0] ?? null,
    [cards, now],
  );
