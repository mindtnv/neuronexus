...
import { NNIcon, NNBtn, NNBadge, NNTag, NNKbd } from '@/components/ui';
...
const QUICK_PROMPT_KEYS = ['article', 'examples', 'related', 'formality'] as const;

const TutorMsg = ({ role, content, loading }: { role: string; content?: string; loading?: boolean }) => {
...
export const NNAITutor = ({ onClose }: { onClose: () => void }) => {
  const t = useT();
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: t('overlays.tutor.greeting'),
    },
  ]);
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      const reply = await (window as any).claude.complete({
        messages: [
      ...
      setMessages(m => [...m, { role: 'assistant', content: t('overlays.tutor.error') }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 600 }}>{t('overlays.tutor.title')}</div>
          <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>{t('overlays.tutor.subtitle')}</div>
        </div>
      ...
      {messages.length <= 2 && !loading && (
        <div style={{ padding: '0 14px 10px', display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {QUICK_PROMPT_KEYS.map(key => {
            const q = t(`overlays.tutor.quickPrompts.${key}`);
            return (
            <button key={key} onClick={() => send(q)} style={{
      ...
      <div style={{
        ...
        <textarea
          ...
          onKeyDown={handleKey}
          placeholder={t('overlays.tutor.placeholder')}
          rows={2}
          style={{
...
export const NNReviewWithTutor = () => {
  const t = useT();
  const bp = useBreakpoint();
  const isMobile = bp === 'mobile';
  ...
  const ratings = [
    { k: '1', label: t('overlays.tutor.review.ratings.again'), interval: '<1m', bg: 'rgba(232,120,138,0.12)', hue: 'var(--rose-500)' },
    { k: '2', label: t('overlays.tutor.review.ratings.hard'),  interval: '8m',  bg: 'rgba(243,182,85,0.12)',  hue: 'var(--amber-500)' },
    { k: '3', label: t('overlays.tutor.review.ratings.good'),  interval: '3d',  bg: 'rgba(154,209,85,0.12)',  hue: 'var(--lime-500)' },
    { k: '4', label: t('overlays.tutor.review.ratings.easy'),  interval: '9d',  bg: 'rgba(85,196,214,0.12)',  hue: 'var(--sky-500)' },
  ];
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
      {/* Review column */}
      <div style={{
        flex: 1, overflow: 'auto', padding: isMobile ? '0 14px' : '0 32px',
        ...
        <div style={{
          ...
          <NNBadge icon="stack" size="sm">{t('overlays.tutor.review.deckBadge')}</NNBadge>
          <div style={{ flex: 1, height: 6, background: 'var(--surface-3)', borderRadius: 3, overflow: 'hidden' }}>
            <div style={{ width: '34%', height: '100%', background: 'var(--lime-500)' }}/>
          </div>
          <span style={{ fontSize: 12, color: 'var(--text-muted)' }} className="mono">{t('overlays.tutor.review.progress')}</span>
        </div>

        <div onClick={() => setRevealed(v => !v)} style={{
          width: '100%', maxWidth: 640, minHeight: isMobile ? 240 : 320,
          borderRadius: 18, background: 'var(--surface)',
          border: '1px solid var(--border)',
          padding: isMobile ? '20px 18px' : '32px 38px', cursor: 'pointer',
          display: 'flex', flexDirection: 'column', position: 'relative',
          boxShadow: 'var(--shadow-lg)',
        }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
            <NNBadge size="xs" tone="neutral">{t('overlays.tutor.review.typeBadge')}</NNBadge>
            <NNTag color="amber">{t('overlays.tutor.review.tag')}</NNTag>
            <div style={{ flex: 1 }}/>
            <NNBtn size="sm" variant="soft" icon="sparkle"
              ...
              {t('overlays.tutor.review.askAI')}
            </NNBtn>
          </div>

          <div style={{ fontFamily: 'var(--font-serif)', fontSize: isMobile ? 36 : 52, lineHeight: 1.1, letterSpacing: -1.2 }}>
            der Nachbar
          </div>
          ...
          {revealed && (
            <>
              ...
              <div style={{ fontSize: 13.5, color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 14, maxWidth: 500 }}>
                <span style={{ color: 'var(--text)' }}>Meine Nachbarn sind sehr freundlich.</span> — {t('overlays.tutor.review.sentenceTranslation')}
              </div>
              <div style={{ padding: '11px 14px', borderRadius: 10, background: 'rgba(167,136,255,0.07)', border: '1px solid rgba(167,136,255,0.2)', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                ...
                <div style={{ fontSize: 12.5, color: 'var(--text-muted)', lineHeight: 1.5 }}>
                  <span style={{ color: 'var(--violet-400)', fontWeight: 500 }}>{t('overlays.tutor.review.mnemonicLabel')}</span>
                  <em>Nacht-bar</em> — {t('overlays.tutor.review.mnemonicText')}
                </div>
              </div>
            </>
          )}
          ...
          {!revealed && (
            <div style={{ fontSize: 13, color: 'var(--text-dim)', display: 'flex', alignItems: 'center', gap: 8 }}>
              <NNKbd>Space</NNKbd> {t('overlays.tutor.review.reveal')}
            </div>
          )}
        </div>

        {revealed && (
          <div style={{ width: '100%', maxWidth: 640, marginTop: 16, display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 10 }}>
            {ratings.map(r => (
              <button key={r.k} style={{
                ...
                <div style={{ fontSize: 11.5, color: 'var(--text-muted)' }} className="mono">{t('overlays.tutor.review.nextIn', { interval: r.interval })}</div>
              </button>
            ))}
          </div>
        )}
